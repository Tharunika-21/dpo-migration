...
function exit_error {
  echo "[`date`] ${*}"
  python3 sendmail.py "[Dev Portal] 11.0 Migration failed for ${tenantName}" "${*}"
  exit 1
}
...
function refreshTenant() {
    ...
    do
        ...
        if [ "$response" == "healthy" ]; then
            console_msg "Backup for $tenantName is completed. Provisioning Tenant in 11.0 version."
            provisionTenant
            break
        else
...
function provisionTenant() {
    ...
    console_msg "Provisioning 11.0 tenant $tenantName"
    ...
    do
        ...
        if [ "$job_status" == "COMPLETED" ]; then
            console_msg "Tenant provisioning in 11.0 is successful."
            registerTenantRepo
            break
        else
            console_msg "11.0 Provisioning for $tenantName is not complete"
            console_msg "Retrying again.."
            sleep 60
        fi
    done
...
function deleteTenant() {
    ...
    do
        job_status=$(curl "${product_tms_url}/jobs/status/${job_id}" -H "Authorization: Basic $product_tms_authToken" -H 'Cookie: tenant=default'| jq -r '.jobStatus' | tr -d '"')
        if [ "$job_status" == "COMPLETED" ]; then
            console_msg "Tenant deletion in 11.0 is successful."
            break
        else
            console_msg "11.0 Delete for $tenantName is not complete"
            console_msg "Retrying again.."
            sleep 60
        fi
    done
...
function restoreIntermediateIndices() {
    ...
    console_msg "[11.0] Restoring 10.15 core indices of $tenantName with restore prefix"
...
function restoreAnalyticIndices() {
    latest_snapshot=$(curl -s -XGET "${es_endpoint}/_snapshot/${repoName}/_all" -u elastic:${es_password} | jq -r ".snapshots[-1].snapshot")
    console_msg "[11.0] Restoring 10.15 metrics index of $tenantName"
...
function reindex() {
    ...
    console_msg "[11.0] Reindex all portal data indices for $tenantName"
    ...
    #else
    #    console_msg "10.15 core data doc count - $doccount1015 & 11.0  core data doc count - $doccount110. Validated that 11.0 doc count is more than or equal to 10.15 doc count for $tenantName."
    #fi
    ...
    console_msg "[11.0] Reindex UMC master indices for $tenantName"
    ...
    console_msg "[11.0] Reindex UMC data indices for $tenantName"
    ...
    #else
    #    console_msg "10.15 UMC data doc count - $doccount1015 & 11.0  UMC data doc count - $doccount110. Validated that 11.0 doc count is more than or equal to 10.15 doc count for $tenantName."
    #fi
    console_msg "[11.0] Reindexing completed for $tenantName..."
    deleteIndices "restored_portal_*"
    licenses=$(curl -s -u elastic:${es_password} -X POST "${es_endpoint}/portal_${tenantName}_umc/_search" -H 'Content-Type: application/json' -d'{
            "query": {
              "term": {
                "index_type": {
                   "value": "licenses"
    ...
    serial_numbers=$(echo "$licenses" | jq -r '.hits.hits[]._source.licenseXml' | sed -n 's:.*<SerialNumber>\(.*\)</SerialNumber>.*:\1:p' | sort)
    if echo "$serial_numbers" | grep -q "0000036219"; then
      license_count=$(echo "$serial_numbers" | wc -l)
      if [ "$license_count" -gt 1 ]; then
        response=$(curl -s -u elastic:${es_password} -X DELETE "${es_endpoint}/portal_${tenantName}_umc/_doc/0000036219")
        echo "Response: $response"
      else
        echo "Only one license found, no deletions needed."
      fi
    else
      echo "Serial number 0000036219 not found"
    fi
    updateSchema
}
...
function updateSchema() {
    ...
    mysql -h $database_endpoint -u $database_user -p$database_password --ssl -se "UPDATE $tms_schema_name.tenant_config SET major_version='11.0' WHERE tenant_name='$tenantName'"
    ...
    if [ "${tier^^}" == "FREE_FOREVER" ]; then
      ...
      mysql -h $database_endpoint -u $database_user -p$database_password --ssl -se "INSERT INTO $tms_schema_name.portal_tenant_config (config_id, common_es_endpoint, major_version, tenant_name, config_keys) SELECT MAX(config_id) + 1,NULL, '11.0', '$tenantName', '$config_keys' FROM $tms_schema_name.portal_tenant_config"
      python3 sendmail.py "[Dev Portal] 11.0 Migration partial success for ${tenantName}" "Data migration to 11.0 is completed for tenant ${tenantName}. Traffic will still be directed to 10.15. Once 10.15 resources are deleted, traffic will be switched to 11.0"
    fi
...
function deleteOldTenant() {
    ...
    python3 sendmail.py "[Dev Portal] 11.0 Migration success for ${tenantName}" "Data migration to 11.0 is completed for tenant ${tenantName}. Traffic is now switched to 11.0"
}
...
case $operation in
  migrate)
      python3 sendmail.py "[Dev Portal] 11.0 Migration is started for ${tenantName}" "Tenant upgrade from current version to 11.0 is started for tenant ${tenantName}. You will be notified once the upgrade completes/fails"
      ...
      if [ "${tenantState}" == "DEPLOYED" ]; then
        ...
        console_msg "Tenant $tenantName is not in DEPLOYED state. Skipping backup to provision 11.0 version of tenant"
        provisionTenant
  ...
  retry)
   ...
   ;;
 deleteOldTenant)
    console_msg "Delete 10.15 Resource and redirecting traffic to 11.0"
    customdomainStatus=$(jq --arg target "customDomain.status" '.[] | select(.name == $target).value' /opt/softwareag/data/json/${tenantName}_110.json | tr -d '"')
    if [ "$customdomainStatus" == "enabled" ]; then
      fqdn=$(jq --arg target "customDomain.domains[0].url" '.[] | select(.name == $target).value' /opt/softwareag/data/json/${tenantName}_110.json | tr -d '"')
    else
